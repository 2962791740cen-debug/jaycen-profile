  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting && !started.current) {
        ...
        // Parse value: handle "~30万", "10%", "40期+", etc.
...
const LandingPage = ({ onEnter }) => {
  ...
  const [phase, setPhase] = useState(0); // 0=initial, 1=narrative1, 2=narrative2, 3=hero, 4=entry
  ...
  useEffect(() => {
    // 打字机效果：narrative 1
    const typeText = (target, setter, delay = 80) => {
  ...
  return (
    <>
      ...
      <div ref={containerRef} onMouseMove={handleMove}
        ...
        style={{ background: C.black, fontFamily: "'Noto Serif SC',serif", perspective: '1200px', cursor: 'none' }}>
        ...
        <div className="absolute top-10 left-10 z-[100]">
          ...
          <div className="text-[10px] tracking-[0.3em] text-white/20 mt-1 font-mono">Profile v3.0 · 2026</div>
        </div>
        {/* 访客计数 */}
        <div className="absolute top-10 right-10 z-[100] text-right pulse-c">
          <div className="text-[10px] font-mono tracking-[0.3em] text-white/30 uppercase">Visitor</div>
          <div className="text-[28px] font-bold mt-1" style={{ color: C.redL, fontFamily: "'Playfair Display',serif" }}>
            #{String(visitCount).padStart(3, '0')}
          </div>
          <div className="text-[10px] font-mono text-white/20 mt-1">你的第 {visitCount} 次访问</div>
        </div>
        ...
        <div className="relative w-full h-full flex justify-center" style={{ transformStyle: 'preserve-3d', zIndex: 10 }}>
          ...
          <div className={`absolute top-0 left-1/2 -translate-x-1/2 h-full z-20 transition-opacity duration-[2000ms] ${closed ? 'flame-line' : ''}`}
            style={{ width: '4px', opacity: closed ? 1 : 0,
              ...
              boxShadow: '0 0 60px rgba(220,38,38,0.8),0 0 18px rgba(252,165,165,0.7),0 0 120px rgba(127,29,29,0.5)' }} />
          <div className="absolute top-[-10%] h-[120%] transition-transform duration-[4200ms] ease-in-out"
          ...
          <div className="absolute inset-0 z-[50] flex flex-col justify-center items-center text-center"
            ...
            {phase === 1 && (
              <div className="max-w-3xl">
                <p className={`typewriter text-3xl md:text-4xl font-medium tracking-[0.15em] text-[#ddd]`} style={{ minHeight: '1.5em' }}>{n1}</p>
                <p className={`mt-6 text-lg md:text-xl text-[#999] tracking-[0.25em] font-light ${n1Sub.length > 0 ? 'typewriter' : ''}`} style={{ minHeight: '1.5em' }}>{n1Sub}</p>
              </div>
            )}

            {phase === 2 && (
              <div className="max-w-3xl">
                <p className={`typewriter text-3xl md:text-4xl font-medium tracking-[0.15em] text-[#ddd]`} style={{ minHeight: '1.5em' }}>{n2}</p>
                <p className={`mt-6 text-lg md:text-xl text-[#999] tracking-[0.25em] font-light ${n2Sub.length > 0 ? 'typewriter' : ''}`} style={{ minHeight: '1.5em' }}>{n2Sub}</p>
              </div>
            )}
            ...
            {phase >= 3 && (
              <div style={{ opacity: phase >= 3 ? 1 : 0, transition: 'opacity 2.5s ease' }}>
                ...
                <h1 style={{
                  fontSize: 'clamp(3rem,6vw,6rem)', fontWeight: 700, letterSpacing: '0.15em', marginBottom: '1.2rem',
                ...
                <div style={{ marginTop: '4rem', opacity: phase >= 4 ? 1 : 0, transition: 'opacity 1.5s ease', pointerEvents: 'auto' }}>
                  <button onClick={handleEnter} className="jc-btn bg-transparent border text-white/80 px-10 py-4 transition-all duration-500"
                    style={{ borderColor: 'rgba(220,38,38,0.3)', fontFamily: "'Noto Serif SC',serif", fontSize: '0.9rem', letterSpacing: '0.45em', cursor: 'none' }}>
                    进入他的世界 →
                  </button>
                </div>
              </div>
    ...
    <div className="min-h-screen text-[#1A1414] font-sans selection:bg-[#DC2626] selection:text-white flex flex-col md:flex-row" style={{ background: C.cream }}>
      <aside className="w-full md:w-64 bg-white border-r border-black/5 flex flex-col h-auto md:h-screen sticky top-0 z-20 flex-shrink-0">
        <div className="p-8 border-b border-black/5">
          ...
          <p className="text-[10px] font-mono text-gray-400 tracking-widest uppercase pl-5">Profile v3.0 · 2026</p>
        </div>
    ...
    <div ref={containerRef} className="h-screen overflow-y-auto text-[#1A1414] font-sans scroll-smooth selection:bg-[#DC2626] selection:text-white" style={{ background: C.paper }}>
      ...
      <nav className="fixed top-0 w-full px-6 py-4 flex justify-between items-center z-40 backdrop-blur-md border-b border-black/5" style={{ background: `${C.paper}E0` }}>
        ...
        <div className="flex items-center gap-3">
          <span className="text-[10px] font-mono text-gray-400">{flippedCount}/3 力量解锁</span>
      ...
      <div className="max-w-5xl mx-auto px-6 pb-32">
        <section className="mb-20">
          <div className="flex items-center gap-3 mb-8 flex-wrap">
            ...
            <span className="text-xs font-mono text-gray-400 ml-2">点击卡片翻转，全部翻完会触发"合成时刻"</span>
          </div>
          ...
          {showSynthesis && (
            <div className="synth-in mt-8">
              <div className="p-10 text-white relative overflow-hidden synth-glow"
                ...
                <div className="relative z-10 max-w-2xl">
                  ...
                  <p className="text-2xl md:text-3xl font-serif font-bold leading-relaxed mb-6">
                    开放 + 系统 + 掌控 = <br />
...
const TimelinePage = ({ onNextModule }) => {
  ...
  const nodes = [
    ...
    { id: 'n5', year: '2025.8—10', label: '关键转折：重建时刻', energy: 7, color: C.redL, tone: `${C.redL}15`,
  ...
  return (
    <div ref={containerRef} className="h-screen overflow-y-auto font-sans scroll-smooth selection:bg-[#DC2626] selection:text-white" style={{ background: C.black, color: '#d8d0cc' }}>
      ...
      <section className="min-h-screen flex flex-col justify-center items-center text-center px-6 pt-20">
        ...
        <p className="text-gray-500 max-w-xl leading-relaxed mb-10">每一次荒废，都不是空白；每一次折返，都带着新的东西回来。</p>

        {/* 能量曲线图 */}
        <div className="w-full max-w-4xl bg-white/[0.02] border border-white/5 p-6 mt-4">
          ...
          <div className="text-[10px] font-mono text-gray-600 text-center mt-2">点击曲线上的节点 · 与下方时间线联动</div>
        </div>
      ...
      <section className="max-w-3xl mx-auto px-6 pb-32 relative">
        ...
        <div className="space-y-6 relative">
          {nodes.map((node) => (
            <div key={node.id} id={`tl-${node.id}`} className="relative pl-12">
              ...
              <div className={`border transition-all duration-400 cursor-pointer overflow-hidden ${activeNode === node.id ? 'border-white/20' : 'border-white/5 hover:border-white/10'}`}
                ...
                <div className="p-6">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="text-[10px] font-mono text-gray-600">能量</div>
      ...
      <div className="max-w-5xl mx-auto px-6 pb-32">
        <section className="mb-16">
          ...
          <div key={activeTrack} className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {t.stats.map((s, i) => (
              <div key={`${activeTrack}-${i}`} className="bg-white p-6 border border-black/5 hover:border-black/10 transition-all hover:-translate-y-0.5">
                <div className="text-[10px] font-mono text-gray-400 uppercase tracking-widest mb-2">{s.label}</div>
                <div className="text-3xl font-black mb-1" style={{ color: t.color, fontFamily: "'Playfair Display',serif" }}>
                  <AnimatedNumber value={s.val} />
                </div>
...
const MindsetPage = ({ onNextModule }) => {
  ...
  const os = [
    ...
    { rank: '04', label: '影响力 · 财富 · 成就', sub: '外显结果层', color: '#9CA3AF', scale: 0.94, desc: '这些是结果，而不是底盘。只有前三层稳了，这层才能持续产出。他已经不再只追结果，而开始意识到承载结果的底层介质。' },
  ];
  ...
  return (
    <div ref={containerRef} className="h-screen overflow-y-auto font-sans scroll-smooth selection:bg-[#DC2626] selection:text-white" style={{ background: C.black, color: '#d8d0cc' }}>
      ...
      <div className="max-w-5xl mx-auto px-6 pb-32">
        ...
        {/* 完成庆祝 */}
        {showCelebration && (
          <div className="burst relative border p-10 mb-10 overflow-hidden" style={{ borderColor: `${C.redL}50`, background: `linear-gradient(135deg, ${C.redL}10 0%, ${C.wine}30 100%)` }}>
            {/* Confetti粒子 */}
            {Array.from({ length: 20 }).map((_, i) => (
              <div key={i} className="confetti"
                style={{
                  ...
                  top: `${Math.random() * 30}%`,
                  background: [C.redL, C.copper, C.amber, C.red][i % 4],
            ...
            <div className="relative z-10 flex items-start gap-5">
              ...
              <div className="flex-1">
                ...
                <p className="text-white font-serif text-2xl leading-relaxed mb-4">
                  "他未来最值得下注的，不一定是单点赢，而是<span style={{ color: C.redL }}>系统赢</span>——当多个能力模块逐渐连成一套系统，才是真正的护城河。"
                </p>
                <p className="text-gray-400 text-sm">你已经看完了五类资产。这五个不是孤立的，而是会互相强化——开放度让他敢起步，迭代力让他不放弃，系统感让他能复利，表达力让他被看见，张力让他被记住。</p>
              </div>
            </div>
          </div>
        )}
    ...
    <div className="min-h-screen text-[#1A1414] font-sans overflow-y-auto selection:bg-[#DC2626] selection:text-white" style={{ background: C.paper }}>
      ...
      <div className="max-w-5xl mx-auto px-6 pb-32">
        <section className="mb-20">
          ...
          <div className="text-center py-5 text-sm font-mono" style={{ background: C.ink, color: '#999' }}>
...
const OutroPage = ({ onRestart }) => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center relative overflow-hidden" style={{ background: C.black }}>
      <style>{`
      ...
      `}</style>
      <div className={`outro-in ${visible ? 'show' : ''} text-center px-8 relative z-10 max-w-3xl`}>
        ...
        <div className={`stat-in ${statsVisible ? 'show' : ''}`}>
          <div className="text-[10px] font-mono tracking-[0.5em] uppercase mb-6" style={{ color: C.redL, opacity: 0.6 }}>Your Journey · 通关报告</div>
          <div className="grid grid-cols-3 gap-4 mb-10">
            {journeyStats.map((s, i) => (
              <div key={i} className="border border-white/10 p-5 bg-white/[0.02]">
                ...
                <div className="text-3xl font-black mb-1" style={{ color: C.redL, fontFamily: "'Playfair Display',serif" }}>
                  {s.val}<span className="text-sm font-normal text-white/40 ml-1">{s.suffix}</span>
                </div>
          ...
          <button onClick={onRestart}
            className="px-8 py-4 border text-white/70 hover:text-white hover:border-[#DC2626] hover:bg-[#DC2626]/10 transition-all text-xs font-mono tracking-[0.4em] uppercase"
  ...
  return (
    <div className="relative" key={current}>
      <style>{`
        ...
        .page-anim { animation: page-fade-in 0.5s ease-out; }
      `}</style>
